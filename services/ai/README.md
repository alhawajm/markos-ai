...
- Images, embeddings, and the generic eight-agent route remain deterministic scaffolding.
...
`OPENAI_API_KEY` is required only when `AI_TEXT_PROVIDER=openai`. Content generation prefers `LLM_PRIMARY_MODEL`; during the transition from older environments, the AI adapter can fall back to the configured long-form model when the gateway still supplies a synthetic `local-*` default. Configure the primary model deliberately rather than relying on that compatibility path. Never place the key in the API service, web environment, repository, command output, logs, screenshots, or test fixtures.
