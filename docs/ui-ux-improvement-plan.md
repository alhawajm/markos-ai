...
| Product or service entry is repetitive for businesses with an existing catalogue | Keep one open Products/Services field as the manual path. Add document-assisted extraction only as a complete upload → analyze → report issues → map → confirm flow; nothing becomes business truth until the owner confirms it. | Next focused pass |
...
| Document-processing policy is unresolved | Decide accepted formats/limits, storage and retention, extraction confidence/issues, field mapping, owner confirmation, retry/replacement behavior, and whether the opportunity is one-time or repeatable before exposing upload. | Product decision |
...
**Checkpoint — 2026-08-30:** The concise greeting, stable seven-step form, two-essential readiness rule, explicit optional skips, open Products/Tone/Priority fields, direct-edit information check, bilingual profile review, and Strategy handoff are implemented and locally/CI verified. The current page intentionally has no document-upload control. `docs/prototypes/onboarding-pass-0-field-contract.html` is preserved as historical design evidence; production behavior and the accepted decision registers now supersede it.
...
2. Define and validate the document-assisted Products/Services policy and one complete owner-confirmed extraction path.
