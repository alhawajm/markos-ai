...
## Current release - 24 September 2026, runtime 0.3.0

Web and both native platforms now share the deployed Railway backend, actual provider embeddings and eight typed AI agents. Account editing, secure workspace switching, email-bound team invitations and roles, knowledge/history, activity, workspace JSON export/erasure, analytics advice and PDF sharing are implemented. Free CPU Motion Reels animate uploaded artwork with exact English/Arabic cards; no paid video API is enabled. This does not synthesize new AI footage. Wan requires suitable GPU infrastructure, and AWS migration remains unprovisioned.

Android ARM64 build `da2f543b-7492-4f8c-bd04-46376ff518be` finished (0.3.0/code 3). iPhone build `2c1fa2a6-5c45-4a54-b527-49d0af02ada0` and submission `3dcfe732-2b38-43cc-a680-068685b6d6f0` finished (0.3.0/build 4). Apple reports VALID and ready for internal testing with the encryption declaration complete. MARKOS's existing group still has only the owner; other Apple apps and credentials were not changed. Expo Sharing requires installing these new binaries. The 0.2.0 Motion update remains available for old installations.

See `apps/mobile/README.md` for install links and `docs/implementation-progress.md` for build/deployment evidence and the remaining work. Physical-device and real Instagram acceptance, push, full account deletion, real monthly email delivery, store listings/review, backup/restore, alert delivery and AWS infrastructure remain outstanding. The Railway credential could deploy but could not enable backups; the owner must enable Daily and Weekly snapshots before relying on recoverability. This checkpoint does not declare the whole product ready for public launch.
