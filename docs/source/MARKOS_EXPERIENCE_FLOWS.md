...
| Content/media | Create normally opens a two-choice greeting: Create with MarkOS or Explore. Campaign handoff bypasses it, preserves the full idea context in one draft, and generates into that same record. Generated copy enters an in-place bilingual review/revision loop before manual editing, approval, media, and scheduling. Calendar adds a bilingual week/month view, an unscheduled queue, dated Campaign plans, and atomic schedule/reschedule/cancel management. | Full queue/recovery states, deployed image-provider proof, content revision history, and all final content-type states. |
...
- With no item selected, Create opens as a compact two-choice greeting rather than an empty editor: **Create with MarkOS** starts grounded generation and **Explore** offers content-idea starting points. Neither choice persists anything merely by being viewed.
- A Campaign suggestion's one **Create** action bypasses the greeting and opens its saved Campaign draft directly. Create identifies its Campaign and week and presents the transferred topic, fixed Instagram platform context, content type, post objective, content pillar, tone, and planning date for review before generation.
- `POST /v1/content/:contentItemId/generate` generates or revises copy in that same workspace-owned Campaign draft. It preserves the item ID, Campaign/week/action identity, planning date, goal, and explicit pillar or tone overrides, so a repeated generation request cannot create a duplicate Campaign draft.
- The broader direct APIs remain `POST /v1/content/generate` and `POST /v1/content/generate-for-slot`. The ordinary blank `POST /v1/content` and saved-record editor contracts continue to exist, but they are no longer separate choices on the normal Create greeting in this presentation slice.
...
- Successful generation opens a dedicated bilingual review surface with platform, content type, call to action, hashtags, planning date, and Campaign context. The owner may mark the saved draft Ready or continue to the ordinary manual editor and media tools.
- `POST /v1/content/:contentItemId/revise` accepts a deliberate revision instruction for an editable AI-generated draft. The AI receives the current full draft, original Campaign/post context, tone lock, and retrieved Vault context. Success updates the same item and records another metered interaction; failure preserves the prior content, and repeated revisions do not create duplicate drafts.
