...
- Current reviewed source includes the refined Calendar and reduced-effort Onboarding checkpoints. The connected Create redesign, IBM Plex/Tangerine Slate foundation, Media Library, and document-assisted onboarding remain prototypes or planned work rather than mounted beta behavior.
...
- Reduced-effort seven-area onboarding: Company and Products are the two essentials; Story, Audience, Competitors, Brand/Tone, and Objectives can be explicitly skipped and resumed. The owner still reviews and approves the generated bilingual business profile before completion.
...
- Full Vault editor/history, document-assisted onboarding, media library, failed-publish queue/retry UI, safeguarded Calendar drag-and-drop, AI consultant, the AN-01 through AN-06 analytics suite, and admin console until their Sunlit surfaces and contracts are implemented and verified. These remain final-system scope rather than retired requirements.
