...
| Reduced-effort onboarding and resolved business profile                                          | **Implemented with focused local coverage; Railway provider proof pending** | The greeting and seven-area wizard keep Company and Products as the two essentials. Story, Audience, Competitors, Brand/Tone, and Objectives can be explicitly skipped and resumed without presenting Vault completeness as a completion gate. Products now have a canonical, versioned Offering Catalog plus an optional document-assisted path for one or two PDF, DOCX, or TXT files. Raw files remain temporary, extracted issues and offerings stay editable, and only explicit owner approval writes the catalog/Vault projection and removes the source files. Offering-document extraction now requires provider-backed AI; local configuration returns a clear sanitized failure instead of speculative line parsing, while the manual field remains available. Focused contract, extraction, persistence, isolation, retention, export, erasure, and client checks pass locally; deployment and one live Railway provider-backed extraction remain unverified. |
...
1. Deploy and live-verify the provider-required document-assisted Products/Services pipeline on Railway, including one successful extraction and one honest failure/recovery state. Then replace the compact Step 3 uploader with the separately reviewed focused Document Analyst experience. Keep broader business-document onboarding and Instagram evidence reconciliation as separate work.
...
| Document-assisted onboarding                    | The first Products/Services slice is implemented in working source: one or two PDF/DOCX/TXT files, 8 MB each/12 MB combined, at most 24-hour temporary retention, editable extraction and issues, retry/discard, and explicit owner approval before canonical writes. It is repeatable; no current provider/cost constraint justifies a one-time opportunity. Broader business-document extraction, OCR, browser/deployed proof, and Instagram reconciliation remain open. |
