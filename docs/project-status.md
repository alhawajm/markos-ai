...
| AI provider inference                                                                            | **Provider connectivity verified; application paths require renewed live verification** | A controlled direct Responses API request from the deployed AI container completed on 2026-08-06 and appeared in the OpenAI project logs, proving that request's key, model access, billing, and outbound connectivity. Current source uses shared strict JSON Schema `responses.create` handling for Campaign generation, onboarding profile resolution, both onboarding document analyzers, and content copy, with bounded timeouts/retries, sanitized diagnostics, and provider token metering. Both document analyzers intentionally reject deterministic local parsing. Focused local/provider tests pass; the full set of application paths is not yet deployed and live-verified. |
