...
| Instagram token refresh and disconnect                                                           | **Implemented and locally/CI verified**                          | Manual and worker refresh paths, expiration handling, guaranteed local cleanup, an explicit Instagram Apps and websites removal action, and signed callback parsing/handling exist. A full production token lifetime, former-token invalidation check, and data-deletion delivery have not been verified. |
