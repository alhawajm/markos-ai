...
| AI service     | Python 3.11 FastAPI service with deterministic local embeddings and fixed strategy, content, image, and eight-agent-shaped responses.                                 | Deploy and secure the service, prove backend connectivity, then add one real provider-backed response. It is not provider-backed today.    |
...
| Instagram token refresh and disconnect                                                           | **Implemented and locally/CI verified; signed callbacks deferred** | Manual and worker refresh paths, expiration handling, guaranteed local cleanup, an explicit Instagram Apps and websites removal action, and signed callback parsing/handling exist. Railway testing verified local disconnect, manual provider removal, and callback delivery, but the live callback failed signature verification with HTTP 403. A full production token lifetime, successful deauthorization/data-deletion handling, and former-token invalidation remain unverified follow-ups. |
...
| MFA and browser bearer-token lifecycle                                                           | **Implemented and locally verified; production deployment pending** | TOTP, access/refresh issuance, rotation, and reuse detection have tests. The browser now keeps refresh credentials in an `httpOnly` cookie, silently renews an in-memory access token, serializes refresh across tabs, and returns terminally expired sessions through interactive login to Profile. |
| AI provider inference                                                                            | **Not implemented**                                              | The FastAPI service does not consume `OPENAI_API_KEY`, call OpenAI, or enforce its configured internal service token. Current outputs are deterministic local scaffolding.                                                               |
...
6. Connect the first real AI capability for onboarding.
...
Deploy the smallest healthy FastAPI service, make the API reach it over an appropriate Railway network path, protect the internal boundary, send one simple request, and receive one real OpenAI-backed response after an authorized API Platform project credential is available. Document health, networking, variables, and troubleshooting.

The current service's `/ai/health` is a shallow process check. `/ai/health/deep` intentionally reports `degraded` with database, provider, and embedding dependencies `not_checked`. `INTERNAL_SERVICE_TOKEN` is configured but not enforced. These are deployment-review inputs, not evidence of a secure production service.

An OpenAI API Platform project and project-authorized credential are separate from a ChatGPT or Codex subscription and its billing. Prefer a project-owned service-account credential over a personal shared key, inject it only into the AI service, and never commit or log it. The current source does not consume `OPENAI_API_KEY`; provider implementation is future product work.
...
3. **AI service deployment:** assess the current deterministic FastAPI service, deploy the smallest healthy service, establish protected backend connectivity, demonstrate one simple request/response, and document blockers. This assignment does not make the onboarding agent production-ready.
