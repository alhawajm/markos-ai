...
The current Campaign duration contract offers 3, 7, 14, 30, 60, and 90 days. The owner also chooses a start date and publishing intensity from one to five publishes per day. Which choices belong to which plans remains unresolved.

## Current architecture overlay

| Area           | Current repository evidence                                                                                                                                           | Target or open work                                                                                                                        |
| -------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------ |
| Web            | Next.js 16 and React 19 in `apps/web`; the adopted Sunlit landing, auth, onboarding, Overview, Campaigns, Create, Insights, Business Profile, and Settings routes call the Fastify API through `NEXT_PUBLIC_API_BASE_URL`. | Restore the complete final-system operational surface recorded in `ui-design-foundation.md` and continue consequential UI review with Mohamed. |
...
| Instagram publishing                                                                             | **Durable showcase bridge implemented and locally verified; not live-provider verified** | The current source keeps the original one-JPEG proof boundary and now adds Post, JPEG Carousel, MP4 Reel, and Business-only Story payloads; half-hour scheduling; asynchronous Publish now; persisted jobs, attempts, leases, and bounded retries; and persistent failure notifications. S3-backed media is signed only for the provider request. Focused local tests pass, but deployment, fresh scope grant, external media fetch, and one successful live publish for each showcased format remain separate gates. |
...
| Reduced-effort onboarding and resolved business profile                                          | **Implemented with focused local and provider-backed browser coverage; full Railway proof pending** | The minimal greeting offers full-business document assistance or the manual seven-area wizard. The document path accepts up to five PDF, DOCX, TXT, JPEG, PNG, or WebP files, extracts a reviewable proposal across the seven modules, reports evidence/issues, supports up to seven owner-confirmed business colors, and converges on the same information check and bilingual Business Profile approval. Files are temporary for at most 24 hours and are removed after analysis approval; no canonical module or Offering Catalog write occurs before approval. The original Products-only analyzer remains available inside that step. Focused contract, provider, persistence, isolation, retention, export, erasure, client, and browser checks pass locally; the full path is not yet verified on Railway. |
| AI provider inference                                                                            | **Text provider connectivity verified; video bridge implemented but not live-verified** | A controlled direct Responses API request from the deployed AI container completed on 2026-08-06 and appeared in the OpenAI project logs. Current text paths use strict structured output with bounded timeouts/retries and sanitized diagnostics. The temporary Sora bridge now adds persisted start/poll/download jobs for 9:16 Reel/Story media and never falls back to synthetic output; focused provider-adapter and persistence tests pass. The video provider, model access, worker-to-AI networking, storage, and a completed application-path generation remain unverified on Railway. |
...
| AI service on Railway                                                                            | **Deployed and reachable; provider-backed Campaign response pending** | On 2026-08-06, API deep health reported the Railway AI dependency `ok`, and the protected, previously named Strategy request reached the AI provider layer. A direct provider request from the same container succeeded, but that old application request returned 503 under the prior adapter. Deploy and verify the current Campaign adapter before calling the path complete. |
...
7. Deploy and live-verify the shared provider-backed Campaign and onboarding-profile response path.
...
Current source can prepare the seven canonical onboarding modules either from owner-entered answers or from an owner-reviewed full-business document proposal. The document analyst uses provider-native file/image inputs, preserves evidence and issues, and writes no canonical knowledge until approval. Company and Products remain the two essentials; the other five areas may be skipped or left absent without fabricating completeness. Both paths generate the same editable bilingual Business Profile, and onboarding completes only after separate profile approval. Khalid owns product behavior and integration; Sarah owns deployment, networking, availability, and secret configuration. Focused local and provider-backed browser checks pass; deployment and one controlled full Railway end-to-end approval remain open.
...
3. Deploy and live-verify the current shared Campaign/onboarding-profile structured-output path through the browser-to-API-to-AI journey.
...
| Campaign entitlement                            | 3/7/14/30/60/90 days are the current product choices; plan mapping is undecided. Publishing intensity supports one to five publishes per day. |
...
| Document-assisted onboarding                    | Full-business first-run assistance is implemented in working source: one to five PDF/DOCX/TXT/JPEG/PNG/WebP files, 8 MB each/20 MB combined, provider-native multimodal analysis, evidence/issues, editable seven-module review, at most 24-hour temporary retention, and explicit approval before canonical writes. The narrower one-or-two-file Products/Services analyzer remains available. Both are repeatable; no current provider/cost constraint justifies a one-time opportunity. Railway proof and future Instagram evidence reconciliation remain open. |
