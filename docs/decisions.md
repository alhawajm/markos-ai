...
When a workspace disconnects Instagram, atomically delete the local encrypted credential, legacy credential fields, and bounded recent-media cache, record the sanitized outcome in the disconnect audit, and return `ACTION_REQUIRED` with Instagram's Apps and websites URL. The UI must tell the user to select **Remove** next to MarkOS AI-IG. Instagram then invalidates provider access and sends the configured, signed deauthorization callback; MARKOS verifies that callback with the app secret and treats a successful callback response as the provider-side completion signal.

Disconnect is not workspace-data erasure. Preserve generated/imported content, publishing history, analytics, and Knowledge Vault learning until the user invokes the separate workspace erasure controls. OAuth authorization URLs disable Facebook Login and force Instagram authentication so a connect or reconnect attempt can use a different professional account. Manual provider removal, signed callback delivery, and former-token invalidation remain controlled live-provider verification gates; local and mocked tests cannot close them.
