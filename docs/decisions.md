...
## 2026-09-03: The showcase bridge uses durable media and publishing jobs

Keep the full manual Create studio available without requiring AI ideation. Place MARKOS AI beside it as an optional primary tool that proposes an editable caption and editable visual direction. The owner must explicitly approve insertion, and an existing studio value must receive a second replacement confirmation. Approved copy moves into the ordinary Caption field; approved visual direction becomes the input to an explicit image or video generation action. Generated output attaches to the same workspace content item and remains manually editable after insertion.

For this time-boxed showcase bridge, support provider-backed 9:16 video generation for Reels and Stories through a persisted asynchronous job. The API owns job state, quota reservation, content attachment, and safe errors; the maintenance worker owns provider start, polling, download, and durable storage. Local and unconfigured environments must fail honestly and must never substitute synthetic bitmap or video output. Use the currently selected Sora provider only as a temporary demonstration dependency: OpenAI has announced that the Videos API and Sora 2 models will shut down on 24 September 2026, so replacement-provider research remains mandatory before any longer-lived product phase.

Expand the Instagram Login publishing adapter to the controlled showcase formats supported by the current permission contract: one JPEG Post, a two-to-ten JPEG Carousel, one vertical MP4 Reel, and a vertical JPEG or MP4 Story for Business accounts. Preserve provider-specific validation and explicit errors rather than silently transforming incompatible media. This is source and local-test capability, not proof of Meta permission grant, App Review, deployed configuration, or a successful live publish for every format.

`Publish now` and scheduled publishing share one persisted job and attempt model. Publish now changes an eligible Ready, Scheduled, or Failed content item into an immediately due job without running the provider inside the browser request. Scheduled inputs use half-hour boundaries. The worker uses leases, one active job per content item, bounded retry attempts, and a recoverable next-attempt timestamp. Terminal failure updates the content item and creates a persistent owner notification; a dry-run worker result must never claim that Instagram published the content.
...
The focused continuation introduces `WORKER_ROLE`: `delivery` owns publishing/video and `maintenance` owns token refresh, Insights, document cleanup, analytics email and usage-period work. Retain `all` as the default for existing installations. Use the same entrypoint/image/database and existing scheduler; deployment of the split must replace the all-task instance rather than add overlapping task owners. This does not move onboarding analysis, campaign generation or Create conversations out of their existing API/AI paths. Role-isolation coverage includes blocked maintenance with continuing delivery; publishing contention coverage exercises three workspaces with mocked Instagram responses, not live provider capacity.
...
Cancellation/rescheduling is allowed for queued and retry-wait publishing, but is rejected once a job is `PROCESSING`, including expired claims awaiting recovery. Schedule edits cancel unclaimed jobs in the same transaction as the content/calendar change. Queue creation and worker claims share the existing content row lock, re-read current state and retain attempt/lease checks. A stale queued job is cancelled rather than failing or publishing a newly edited schedule. Scheduled-job keys include the existing content revision so a deliberate return to a previously cancelled time can create a fresh job. No new schema or external cancellation API is introduced. Legacy synchronous operator publishing routes remain outside the worker claim model; they are not the supported browser Publish now path and should not be used alongside the worker for the same content.

For video submission, distinguish an unknown result from a failed render. Persist `STARTING` before submission and the provider job ID immediately after receiving it. A recovered `STARTING` job without that ID, or an ambiguous submission error, stops with `AI_VIDEO_START_RESULT_UNKNOWN`; it is not automatically resubmitted. The existing failed-job retry action is an intentional new generation, accompanied by an English/Arabic warning about a possible duplicate video and additional provider charge. Known rejections retain their classified errors and safe retry behavior. Polling/download failures with a saved ID resume the same provider job. Disable SDK retries for video requests so retries are owned by the worker, retain the 120-second AI/130-second API defaults, and do not equate the request timeout with the total rendering duration. This makes no claim about provider billing for an unknown attempt. Automated coverage uses mocked provider responses and HTTP transport, not live generation.
...
Create conversation edits to a Reel must populate the existing `visualDirection` used by video generation. Preserve a returned structured `reelScript`; if it arrives without a usable visual direction, derive that direction from its hook and beats in the same atomic draft update. Reject an oversized fallback instead of silently truncating it. Explicit provider direction takes precedence. Keep manual duration/media generation controls unchanged. Generate edit confirmations on the application side only after successful persistence; reject empty edit payloads and retain existing conflict/failure messages. Discussion remains conversational and must not claim a draft mutation. No new content fields, media generation actions, or Intelligence functionality are introduced.

## 2026-09-13: Insights discovery, partial results, and period ownership

Discover Instagram media independently of local drafts and retain its provider media ID in the existing analytics representation. Link by the final published media ID only; do not infer matching from captions or create local draft records for external content. Account aggregates own reporting-window totals, provider time series own daily trends, and media insights remain lifetime values for content published in the selected window. Preserve measured zero separately from unavailable data. Save useful partial results and explicit diagnostics; do not describe major request failures as complete synchronization. See `docs/analysis/showcase-insights-2026-09-13.md` for the read-only test-account evidence and limits.

## 2026-09-13: Create assistant edits individual Details fields

Expose the existing `contentPillar`, `campaignGoal` (the post objective), and `tone` fields in the conversation edit contract alongside `brief`. The assistant already receives these saved values, but previously could not return mutations for them. Apply each reviewed value to its own field through the existing atomic revision-checked update; do not use Brief as a substitute. Null means unchanged and an empty string clears a field. Keep current field limits and server-generated save confirmations, preserve unrelated copy, and retain compatibility with older stored AI responses. This does not edit the parent campaign or introduce new content fields. Focused API, provider, and browser tests cover separate persistence, invalid edits, and refreshed/reloaded editor inputs.

## 2026-09-13: Publish now priority within the existing maintenance tick

Refresh due Instagram credentials, then process due publish jobs before document cleanup, email, usage maintenance, and Insights synchronization. Reproduced that an email or Insights exception previously prevented publishing from running at all. Keep one timer, its running guard, durable leases, retry policy, and duplicate protection. This changes task order only. Publish now retains its existing MFA step-up requirement; tests explicitly distinguish authorization rejection before queueing from a queued Reel that creates a container, waits for processing, publishes, and persists the final media ID. This code-level defect is not proof of the cause of an individual production attempt.
## 2026-09-13 — Business Profile becomes the permanent knowledge editor

Implement the approved maintenance prototype on `feat/business-profile-maintenance`, based on the merged color-system refinement. Tabs are Marketing Strategy (the default), Products & Services, Audience & Market, Brand & Voice, and Business. Use `/{locale}/app/knowledge?tab=...`; an absent or unknown tab opens Marketing Strategy. The September 7 product decision supersedes the August 20 return-to-onboarding maintenance requirement.

A reserved `COMPANY/authoritative-profile` record reuses existing workspace-scoped Vault storage and revision history as one authoritative profile document. It is excluded from ordinary Vault lists, completeness and retrieval. Section entries are derived readable projections. Products & Services retains the existing catalog and offering revisions. There is no new profile table, offering store or schema migration. Existing workspace facts are adopted on the first authoritative write; no records are deleted or reset.

Owner edits require an expected profile/catalog revision and commit facts, projections and stale-summary retirement atomically under a workspace knowledge lock. Maintenance saves make no AI requests. Retrieval lazily indexes committed unembedded facts; version checks reject late embeddings, and a provider outage falls back to current readable facts. Legacy onboarding writers share persistence and preserve approved/COMPLETE status. Omitted fields do not clear unrelated data. Empty optional brand/tone halves remove their old projections. A reviewed business name becomes the same name used in company facts. Historical approved AI interactions remain unchanged; after maintenance, onboarding's compatibility summary is assembled from the current saved facts without AI or invented translation.
...
## 2026-09-13 — Scope unfinished onboarding drafts to their owner

Manual onboarding caches use a versioned user-and-workspace key. The previous browser-wide v1/v2/v3 caches have no reliable ownership and are discarded rather than migrated into the current account. Saved server facts are unaffected; an old unsaved browser-only draft must be re-entered. Restore and autosave require the same authenticated owner, and the route unmounts prior-owner state while loading another workspace. Approved-profile edit mode does not write into the unfinished onboarding cache. A same-browser regression covers new accounts, workspace changes, same-owner refresh recovery and returning to the original owner; API checks keep fresh-account profile/onboarding responses separate from another workspace's facts.

## 2026-09-13 — Verification confirms identity; the signup tab continues onboarding
...
## 2026-09-24: Release status must reflect actual delivery

The monthly report email adapter remains simulated. Return `delivered: false` and `skippedReason: DRY_RUN`, record new audits as `MONTHLY_ANALYTICS_PDF_EMAIL_SIMULATED`, and count zero deliveries in worker summaries. Existing historical dry-run records are not inbox evidence. Activating real report email requires an owner preference and durable handling of ambiguous provider outcomes; do not automatically email every existing workspace merely because SendGrid is configured for authentication.

The native application uses platform transport and secure storage. The existing platform-encryption declaration used for TestFlight build 3 also applies to 0.3.0/build 4. Completed that declaration only on the verified MARKOS build and set `ios.config.usesNonExemptEncryption: false` for future builds, following [Apple's encryption export guidance](https://developer.apple.com/documentation/security/complying-with-encryption-export-regulations). This metadata does not add a native module or change the JavaScript runtime contract. No other app, distribution certificate or provisioning profile was modified.

## 2026-09-24: Native Create and campaign completion

Give Story its own visible format and choose image/video per Story slot. Post and Carousel generate images; Reel generates video. Native conversion uses the existing revision-checked preview/confirmation contract. Assistant preparation preserves the chosen format. The primary action guides missing caption/media work before offering Mark Ready; approval and scheduling remain separate from generation, as on the website.
