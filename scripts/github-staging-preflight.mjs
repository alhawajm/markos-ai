...
const ecsCoreVariables = ["STAGING_AWS_ROLE_ARN", "STAGING_AWS_REGION", "STAGING_ECS_CLUSTER"];
const ecsServiceVariables = ["STAGING_WEB_SERVICE", "STAGING_API_SERVICE", "STAGING_WORKER_SERVICE", "STAGING_AI_SERVICE"];
...
if (strict && report.status !== "ready") {
  console.error(`GitHub staging preflight failed: ${report.blockers.join("; ")}`);
  process.exit(1);
}
...
function buildReport(repository) {
  ...
  const variables = environment.exists ? readEnvironmentVariables(repository) : [];
  ...
  const ecsServices = summarizeVariableGroup(ecsServiceVariables, variableNames);
  ...
  if (!ecsConfigured) {
    blockers.push(ecsCore.ready ? "No ECS service variables configured" : `Missing ECS core variables: ${ecsCore.missing.join(", ")}`);
  }

  return {
    gate: "staging",
    event: "github-staging-preflight",
    status: environment.exists && smoke.ready && ecsConfigured ? "ready" : "blocked",
    repository,
    environment: "staging",
    startedAt,
...
function readEnvironment(repository) {
  try {
    const environment = ghJson(["api", `/repos/${repository}/environments/staging`]);
    return {
      exists: true,
      name: environment.name,
      protectedBranches: Boolean(environment.deployment_branch_policy?.protected_branches),
      customBranchPolicies: Boolean(environment.deployment_branch_policy?.custom_branch_policies)
    };
  } catch (error) {
    return {
      exists: false,
...
function createEnvironment(repository) {
  execFileSync("gh", ["api", "--method", "PUT", `/repos/${repository}/environments/staging`], {
    encoding: "utf8",
...
function readEnvironmentVariables(repository) {
  try {
    const response = ghJson(["api", `/repos/${repository}/environments/staging/variables`, "--paginate"]);
    return Array.isArray(response.variables) ? response.variables.map((variable) => ({ name: variable.name })) : [];
  } catch {
    return [];
  }
}
