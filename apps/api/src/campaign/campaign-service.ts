import type { Campaign, ContentItem, ContentStatus, ContentType, Prisma } from "@prisma/client";
import type {
  CampaignBrief,
  CampaignPackage,
  CampaignPackageItem,
...
import { generateWorkspaceContent, toContentRecord } from "../content/content-service";
...
export class CampaignContextMissingError extends Error {
  constructor() {
    super("Complete at least one Vault section before generating a campaign package");
  }
}
...
export async function listCampaignPackages(
  ...
  return Promise.all(campaigns.map((campaign) => toCampaignPackageRecord(campaign)));
}
...
export async function generateCampaignPackage(
  ...
  const [vaultContext, catalogContext] = await Promise.all([
  ...
  try {
    ...
    if (contentItems.length === 0) {
      throw new CampaignStateError("Campaign generation returned no content items");
    }

    const generatedPackage = validateCampaignPackage(buildCampaignPackage(brief, contentItems, plan, catalogContext, vaultContext));
    ...
    await prisma.campaign.update({
      ...
      data: {
        status: "ARCHIVED",
        rationale: error instanceof Error ? error.message : "Campaign generation failed"
      }
    });
    throw error;
  }
...
export async function approveCampaignPackage(workspaceId: string, campaignId: string): Promise<CampaignPackageRecord> {
  ...
  const invalidItem = contentItems.find((item) => !["DRAFT", "IN_REVIEW", "APPROVED"].includes(item.status));

  if (invalidItem) {
    throw new CampaignStateError(`Content item ${invalidItem.id} is already ${invalidItem.status.toLowerCase()}`);
  }
...
export async function rejectCampaignItem(
  ...
  if (["SCHEDULED", "ARCHIVED"].includes(campaign.status)) {
    throw new CampaignStateError("Scheduled or archived campaigns cannot accept item feedback");
  }
  ...
  const updated = await prisma.$transaction(async (tx) => {
    ...
    return tx.campaign.update({
      ...
      data: {
        status: "IN_REVIEW",
        rejectedIdeas: [...rejectedIdeas, feedback] as unknown as Prisma.InputJsonValue
      }
    });
  });
...
export async function scheduleCampaignPackage(
  ...
  if (campaign.status !== "APPROVED") {
    throw new CampaignStateError("Approve the campaign package before scheduling it");
  }
  ...
  if (invalidItem) {
    throw new CampaignStateError(`Content item ${invalidItem.id} must be approved before scheduling`);
  }
  ...
  const updated = await prisma.$transaction(async (tx) => {
    for (const entry of schedule) {
      ...
      await addToContentCalendar(tx, workspaceId, updatedItem.id, entry.scheduledAt);
    }
...
async function findCampaign(workspaceId: string, campaignId: string): Promise<Campaign> {
...
async function listCampaignContentItems(workspaceId: string, campaignId: string): Promise<ContentItem[]> {
...
async function toCampaignPackageRecord(
  ...
  const contentItems =
    contentRecords ??
    (await listCampaignContentItems(campaign.workspaceId, campaign.id)).map((item) => toContentRecord(item));
...
function toCampaignRecord(campaign: Campaign, contentItems: ContentRecord[]): CampaignRecord {
  ...
  return {
    ...
    ...(campaignPackage === undefined ? {} : { package: syncPackageStatuses(campaignPackage, contentById) }),
    ...
    ...(campaign.startsAt === null ? {} : { startsAt: campaign.startsAt.toISOString() }),
    ...(campaign.endsAt === null ? {} : { endsAt: campaign.endsAt.toISOString() }),
    ...(campaign.generatedAt === null ? {} : { generatedAt: campaign.generatedAt.toISOString() }),
    ...(campaign.approvedAt === null ? {} : { approvedAt: campaign.approvedAt.toISOString() }),
...
function buildCampaignPlan(
  ...
  return Array.from({ length: brief.contentCount }, (_, index) => ({
    ...
    contentType: brief.contentTypes[index % brief.contentTypes.length] ?? "POST",
...
function buildCampaignPackage(
  ...
  const schedule = contentItems.map((item, index) => {
    ...
    const scheduledAt = dateAtCampaignTime(campaignStartDate(brief.startDate), planItem?.day ?? index + 1, defaultCampaignTime);
...
function validateCampaignPackage(campaignPackage: CampaignPackage): CampaignPackage {
  const parsed = campaignPackageSchema.safeParse(campaignPackage);

  if (!parsed.success) {
    const issues = parsed.error.issues.map((issue) => `${issue.path.join(".") || "package"}: ${issue.message}`).join("; ");
    throw new CampaignStateError(`Campaign package validation failed: ${issues}`);
  }

  return parsed.data as CampaignPackage;
}

function buildCampaignAngles(brief: CampaignBrief, catalogContext: VaultRagChunk[], vaultContext: VaultRagChunk[]): string[] {
  const selectedOffer = findCatalogChunk(catalogContext, "offer", true);
  const selectedProduct = findCatalogChunk(catalogContext, "product", true) ?? findCatalogChunk(catalogContext, "product", false);
  const productName = stringValue(selectedProduct?.value.name) ?? "the priority offer";
  const offerTitle = stringValue(selectedOffer?.value.title);
  const benefits = stringArrayValue(selectedProduct?.value.benefits).slice(0, 3);
  const audience = brief.audience ?? firstVaultSignal(vaultContext, "AUDIENCE") ?? "your highest-fit Instagram audience";
  const angles: string[] = [];
  ...
  angles.push(`Turn the campaign into a short proof-led sequence for ${audience}.`);
...
function buildContentTopic(
  ...
  const parts = [
    ...
    selectedProduct === undefined ? undefined : `Product: ${stringValue(selectedProduct.value.name) ?? "selected product"}`,
    selectedOffer === undefined ? undefined : `Offer: ${stringValue(selectedOffer.value.title) ?? "selected offer"}`,
    firstVaultSignal(vaultContext, "BRAND") === undefined ? undefined : `Brand fact: ${firstVaultSignal(vaultContext, "BRAND")}`
  ];

  return parts.filter((part): part is string => typeof part === "string").join("\n");
}

function buildCampaignObjectives(brief: CampaignBrief, catalogContext: VaultRagChunk[]): Array<{ label: string; value: string }> {
  const selectedOffer = findCatalogChunk(catalogContext, "offer", true);
  ...
  return [
    ...
    },
    {
      ...
      value: priceMinor === undefined ? "Campaign-ready" : `${formatBhd(priceMinor)} offer`
    }
  ];
}

function buildRationale(brief: CampaignBrief, catalogContext: VaultRagChunk[], vaultContext: VaultRagChunk[]): string {
  const productName =
    stringValue(findCatalogChunk(catalogContext, "product", true)?.value.name) ??
    stringValue(findCatalogChunk(catalogContext, "product", false)?.value.name) ??
    "the selected business priority";
  const audience = brief.audience ?? firstVaultSignal(vaultContext, "AUDIENCE") ?? "the priority audience";
...
function buildScheduleForItems(
  ...
  const packageSchedule = new Map((campaignPackage?.schedule ?? []).map((entry) => [entry.contentItemId, entry]));
  const base = input.startDate === undefined ? undefined : parseDate(input.startDate);

  return contentItems.map((item, index) => {
    const packageEntry = packageSchedule.get(item.id);
    const proposed = input.startDate === undefined && packageEntry?.scheduledAt !== undefined ? parseDate(packageEntry.scheduledAt) : undefined;
    const day = packageEntry?.day ?? index + 1;
    const scheduledAt = proposed !== undefined && proposed > new Date() ? proposed : dateAtCampaignTime(base ?? tomorrow(), day, input.time);
...
function syncPackageStatuses(campaignPackage: CampaignPackage, contentById: Map<string, ContentRecord>): CampaignPackage {
  return {
    ...campaignPackage,
    items: campaignPackage.items.map((item) => {
      ...
      return {
        ...
        ...(content.scheduledAt === undefined ? {} : { scheduledAt: content.scheduledAt })
      };
    }),
...
function parseCampaignPackage(value: Prisma.JsonValue | null): CampaignPackage | undefined {
  if (!isRecord(value)) {
    return undefined;
  }
...
function parseCampaignBrief(value: Prisma.JsonValue): CampaignBrief {
  ...
  return {
    ...
    contentTypes: Array.isArray(value.contentTypes) ? uniqueContentTypes(value.contentTypes) : ["POST"],
...
function mergeCalendarPlan(value: Prisma.JsonValue | undefined, contentItemId: string): { scheduledContentIds: string[] } {
  const current =
...
function dateBounds(campaignPackage: CampaignPackage): [Date | undefined, Date | undefined] {
  const dates = campaignPackage.schedule.map((entry) => parseDate(entry.scheduledAt)).sort((a, b) => a.getTime() - b.getTime());
...
function findCatalogChunk(catalogContext: VaultRagChunk[], sourceType: "offer" | "product", selected: boolean): VaultRagChunk | undefined {
  return catalogContext.find((chunk) => chunk.value.sourceType === sourceType && Boolean(chunk.value.selectedForGeneration) === selected);
}

function firstVaultSignal(vaultContext: VaultRagChunk[], section: string): string | undefined {
...
function uniqueContentTypes(values: unknown[]): ContentType[] {
  const filtered = values.filter((value): value is ContentType => contentTypes.includes(value as ContentType));
...
function contentTypeValue(value: unknown): ContentType {
  return contentTypes.includes(value as ContentType) ? (value as ContentType) : "POST";
}
...
const contentStatuses: ContentStatus[] = ["DRAFT", "IN_REVIEW", "APPROVED", "SCHEDULED", "PUBLISHED", "FAILED"];
