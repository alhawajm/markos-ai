...
export function decodeEncryptionKey(encodedKey: string): Buffer {
  ...
  if (key.length !== 32 || key.toString("base64") !== encodedKey) throw new CredentialEncryptionError();
  return key;
}
...
export function decryptCredential(envelope: string, encodedKey: string): string {
  try {
    ...
    if (version !== VERSION || !iv || !tag || !ciphertext || extra) throw new Error();
    ...
    const decipher = createDecipheriv(ALGORITHM, decodeEncryptionKey(encodedKey), ivBytes);
...
export function redactSecrets(value: unknown): unknown {
  ...
  return Object.fromEntries(
    Object.entries(value).map(([key, item]) =>
      /(?:access[_-]?token|app[_-]?secret|encryption[_-]?key|state[_-]?secret)/i.test(key) ? [key, "[REDACTED]"] : [key, redactSecrets(item)]
    )
  );
}
