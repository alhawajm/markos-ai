...
export interface OAuthStateStore {
  consume(
    nonce: string,
  ): Promise<"consumed" | "already_consumed" | "not_found_or_expired">;
...
export async function issueOAuthState(input: {
  ...
  if (!ALLOWED_RETURN_PATHS.has(input.returnTo))
    throw new OAuthStateError("return_path_invalid");
...
export async function consumeOAuthState(input: {
  ...
  if (
    ...
    throw new OAuthStateError("binding_invalid");
...
export function verifyOAuthState(
  ...
  try {
    ...
    if (actual.length !== expected.length || !timingSafeEqual(actual, expected))
      throw new OAuthStateError("signature_invalid");
