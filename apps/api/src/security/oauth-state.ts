...
export async function issueOAuthState(input: {
  ...
  if (!ALLOWED_RETURN_PATHS.has(input.returnTo)) throw new OAuthStateError();
...
export async function consumeOAuthState(input: {
  ...
  if (!(await input.store.consume(claims.nonce))) throw new OAuthStateError();
...
function sign(payload: string, secret: string): string {
  if (Buffer.byteLength(secret) < 32) throw new OAuthStateError();
