...
import { completeInstagramOAuth, createInstagramOAuthStart } from "../src/workspace/instagram-oauth-service";
import {
...
describeInstagramDatabase("Instagram encrypted persistence integration", () => {
  beforeAll(async () => {
    if (!encryptionKey) throw new Error("INSTAGRAM_TOKEN_ENCRYPTION_KEY is required");
  });
  afterAll(async () => {
  ...
  it("encrypts, reconnects idempotently, blocks active cross-workspace ownership, and releases ownership on disconnect", async () => {
    ...
    const stored = await prisma.instagramConnectionCredential.findUniqueOrThrow({ where: { workspaceId: first } });
    expect(stored.encryptedAccessToken).not.toContain("long-lived-one");
    expect(decryptCredential(stored.encryptedAccessToken, encryptionKey)).toBe("long-lived-one");
    ...
    await expect(persist(second, "other-workspace")).rejects.toBeInstanceOf(InstagramConnectionConflictError);
    await expect(disconnectSecureInstagram(first, actorId)).resolves.toMatchObject({
    ...
    await expect(persist(first, "same-workspace-again")).resolves.toMatchObject({ connected: true });
    await disconnectSecureInstagram(first, actorId);
    await expect(persist(second, "legitimate-transfer")).resolves.toMatchObject({ connected: true });
  });
  ...
  it("handles refresh eligibility, success, transient failure, expiration, and post-disconnect prevention", async () => {
    ...
    await persist(id, "current", new Date(now.getTime() - 2 * 86_400_000), new Date(now.getTime() + 10 * 86_400_000), "17841400000000001");
    const transient = new InstagramBasicClient(async () => new Response("{}", { status: 503 }));
    await expect(refreshSecureInstagram({ workspaceId: id, client: transient, now })).resolves.toMatchObject({
      refreshed: false,
      reason: "INSTAGRAM_REFRESH_FAILED"
    });
    expect((await getDecryptedCredential(id))?.accessToken).toBe("current");
    const success = new InstagramBasicClient(async () => response({ access_token: "replacement", expires_in: 3600 }));
    await expect(refreshSecureInstagram({ workspaceId: id, client: success, now })).resolves.toMatchObject({ refreshed: true });
    expect((await getDecryptedCredential(id))?.accessToken).toBe("replacement");
    await disconnectSecureInstagram(id, actorId);
    await expect(refreshSecureInstagram({ workspaceId: id, client: success, now })).resolves.toEqual({ refreshed: false, reason: "INSTAGRAM_NOT_CONNECTED" });
  });
  ...
  it("requires provider removal, finishes local cleanup, and preserves workspace history", async () => {
    ...
    expect(await prisma.instagramRecentMedia.count({ where: { workspaceId: id } })).toBe(0);
    expect(await prisma.contentItem.findUnique({ where: { id: content.id } })).not.toBeNull();
    expect(
  ...
  it("enforces application-role RLS for states, credentials, and media", async () => {
    const first = await workspace();
    const second = await workspace();
    await persist(first, "first-token", undefined, undefined, "17841400000000101", [{ id: "media-first", mediaType: "IMAGE" }]);
    await persist(second, "second-token", undefined, undefined, "17841400000000102", [{ id: "media-second", mediaType: "VIDEO" }]);
    await createPrismaOAuthStateStore(actorId, first).put(`first-nonce-${randomUUID()}`, new Date(Date.now() + 60_000));
    await createPrismaOAuthStateStore(actorId, second).put(`second-nonce-${randomUUID()}`, new Date(Date.now() + 60_000));
    ...
    const counts = await prisma.$transaction(async (tx) => {
      ...
      const [connections] = await tx.$queryRaw<Array<{ count: bigint }>>`SELECT count(*) FROM instagram_connection_credentials`;
      const [media] = await tx.$queryRaw<Array<{ count: bigint }>>`SELECT count(*) FROM instagram_recent_media`;
      const [states] = await tx.$queryRaw<Array<{ count: bigint }>>`SELECT count(*) FROM oauth_state_nonces`;
      return [Number(connections?.count), Number(media?.count), Number(states?.count)];
    });
    ...
    expect(
      await asApplicationRole(first, (tx) =>
        tx.$executeRawUnsafe(`UPDATE instagram_connection_credentials SET username = 'blocked' WHERE id = '${secondConnection.id}'`)
      )
    ).toBe(0);
    expect(
      await asApplicationRole(first, (tx) => tx.$executeRawUnsafe(`DELETE FROM instagram_connection_credentials WHERE id = '${secondConnection.id}'`))
    ).toBe(0);
    expect(await asApplicationRole(first, (tx) => tx.$executeRawUnsafe(`DELETE FROM instagram_recent_media WHERE id = '${secondMedia.id}'`))).toBe(0);
    expect(await asApplicationRole(first, (tx) => tx.$executeRawUnsafe(`DELETE FROM oauth_state_nonces WHERE id = '${secondNonce.id}'`))).toBe(0);
    expect(
      await asApplicationRole(first, (tx) =>
        tx.$executeRawUnsafe(`UPDATE oauth_state_nonces SET "consumedAt" = now() WHERE id = '${secondNonce.id}' AND "consumedAt" IS NULL`)
      )
    ).toBe(0);
  });
  ...
  it("consumes a nonce atomically and prevents duplicate callback exchange", async () => {
    ...
    expect((await Promise.all([store.consume(atomicNonce), store.consume(atomicNonce)])).sort()).toEqual(["already_consumed", "consumed"]);
  ...
  ] as const)("classifies and rolls back the %s boundary", async (failedStage) => {
    const id = await workspace();
    await expect(
      persistInstagramConnection({
        workspaceId: id,
  ...
  it("enforces concurrent uniqueness and rolls back failed persistence and disconnect", async () => {
    ...
    await expect(
      persist(rollback, "never-committed", undefined, undefined, "17841400000000302", [{ id: "bad-media", mediaType: "IMAGE", timestamp: "not-a-date" }])
    ).rejects.toThrow();
    expect(
      await prisma.instagramConnectionCredential.findUnique({
    ...
    await persist(disconnectRollback, "retained", undefined, undefined, "17841400000000303");
    await prisma.workspace.delete({ where: { id: disconnectRollback } });
    await expect(disconnectSecureInstagram(disconnectRollback, actorId)).rejects.toThrow();
    expect(
      await prisma.instagramConnectionCredential.findUnique({
...
async function asApplicationRole<T>(workspaceId: string, callback: (tx: Prisma.TransactionClient) => Promise<T>): Promise<T> {
