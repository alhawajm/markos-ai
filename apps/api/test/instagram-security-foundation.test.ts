...
describe("Instagram provider contract", () => {
  it("uses only Instagram Login v25.0 and the basic business scope", () => {
    expect(INSTAGRAM_AUTHORIZATION_URL).toBe(
      "https://www.instagram.com/oauth/authorize",
    );
    expect(INSTAGRAM_SHORT_LIVED_TOKEN_URL).toBe(
      "https://api.instagram.com/oauth/access_token",
    );
    expect(INSTAGRAM_LONG_LIVED_TOKEN_URL).toBe(
  ...
  it("accepts a canonical redirect and rejects ambiguous credentials or fragments", () => {
    expect(
      canonicalRedirectUri("https://api.markos.test/v1/instagram/callback"),
    ).toBe("https://api.markos.test/v1/instagram/callback");
    expect(() =>
      canonicalRedirectUri("https://user:pass@api.markos.test/callback"),
    ).toThrow();
    expect(() =>
      canonicalRedirectUri("https://api.markos.test/callback#other"),
    ).toThrow();
  });
});
...
describe("OAuth state", () => {
  async function state(
    overrides: Partial<Parameters<typeof issueOAuthState>[0]> = {},
  ) {
  ...
  it.each([
    ...
    [
      "expired",
      () => state({ now: new Date("2026-01-01T00:00:00Z"), ttlSeconds: 1 }),
    ],
    ["cross-user", () => state()],
    ...
    const now =
      kind === "expired" ? new Date("2026-01-01T00:00:02Z") : undefined;
    await expect(
      consumeOAuthState({
        state: value,
        secret,
  ...
  it("rejects unsafe return destinations", async () => {
    await expect(
      state({ returnTo: "https://evil.test/steal" }),
    ).rejects.toBeInstanceOf(OAuthStateError);
    await expect(state({ returnTo: "//evil.test" })).rejects.toBeInstanceOf(
      OAuthStateError,
    );
  });
});
...
describe("credential handling", () => {
  it("round trips with randomized authenticated ciphertext", () => {
    ...
    expect(() => decryptCredential(`${first.slice(0, -1)}x`, key)).toThrow(
