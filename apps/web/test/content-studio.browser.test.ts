...
async function setup(items: ContentRecord[] = [], media: MediaAssetRecord[] = [asset()]) {
  ...
  await page.route(/^http:\/\/(?:localhost|127\.0\.0\.1):4000\//, async (route) => {
    ...
    const job = () => ({ id: "video-job", contentItemId: state.items[0]?.id, status: state.jobStatus, progress: 40 });
    ...
    if (path === "/v1/media-generation/video-job") {
      ...
      if (state.jobStatus === "COMPLETED") state.items[0] = { ...state.items[0]!, mediaIds: ["photo", "video-result"] };
    ...
    if (match && item) {
      ...
      if (action === "/media") item.mediaIds = [...new Set([...item.mediaIds, body.mediaAssetId])];
      if (action?.startsWith("/media/") && method === "DELETE") item.mediaIds = item.mediaIds.filter((id) => id !== action.split("/")[2]);
...
describe("unified Create", () => {
  it("reconnects to an in-progress conversation after refresh without sending again", async () => {
    ...
    try {
      ...
      await page.getByText("MARKOS is working. You can leave and return to this conversation.", { exact: true }).waitFor();
      await page.reload();
  ...
  it("generates a complete caption into a new blank draft and uses spare width for the conversation", async () => {
    ...
    try {
      ...
      const output = await page.locator(".studio-output").boundingBox();
      expect(chat!.width).toBeGreaterThan(output!.width + 100);
  ...
  it("revises the same manual Campaign draft, saving edits first and locking competing actions", async () => {
    ...
    try {
      ...
      await page.getByRole("button", { name: "Send to MARKOS", exact: true }).click();
      await page.waitForFunction(() => document.querySelector(".studio-conversation-log")?.textContent?.includes("MARKOS is working"));
  ...
  it("uploads to the same saved draft and attaches library media for a carousel", async () => {
    ...
    try {
      ...
      await page.getByRole("dialog", { name: "Media Library" }).getByRole("button", { name: "photo.jpg", exact: true }).click();
  ...
  it("keeps the outer preview ratio across formats, desktop sizes and Arabic", async () => {
    const { page, close } = await setup([draft({ mediaIds: ["photo", "square"] })], [asset(), asset("square", 1080, 1080)]);
    try {
      await open(page, "saved-post");
      for (const type of ["POST", "CAROUSEL", "REEL", "STORY"]) {
        await page.getByRole("button", { name: "Edit caption", exact: true }).click();
        ...
        for (const viewport of [
          ...
          expect(box!.width / box!.height).toBeCloseTo(360 / 730, 2);
          expect(box!.width).toBeGreaterThan(viewport.height === 768 ? 320 : 380);
  ...
  it("retries transient worker polling and attaches completed media without replacing unsaved copy", async () => {
    const { page, state, close } = await setup([draft({ contentType: "REEL" })], [asset(), asset("video-result", 1080, 1920)]);
    ...
    try {
      ...
      await page.getByRole("button", { name: "Edit media", exact: true }).click();
      await page.getByRole("button", { name: "Remove video-result.jpg", exact: true }).waitFor({ timeout: 15000 });
      await page.getByRole("button", { name: "Edit caption", exact: true }).click();
