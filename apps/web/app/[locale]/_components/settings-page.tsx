...
export function SettingsPage({ locale }: { locale: Locale }) {
  ...
  if (!sessionChecked) {
    return (
      <main
        className="sunlit-theme sunlit-app grid min-h-screen place-items-center px-6"
      ...
      >
        <section className="sunlit-panel max-w-md rounded-[2rem] p-9 text-center">
          ...
          {sessionCheckFailed ? (
            <button
              className="sunlit-primary mt-5 rounded-xl px-5 py-3 font-black"
  ...
  const firstName =
    session?.user.fullName.split(/\s+/)[0] ||
    (locale === "ar" ? "الحساب" : "Account");

  return (
    <main
      className="sunlit-theme sunlit-app min-h-screen min-w-0 overflow-x-clip"
      dir={locale === "ar" ? "rtl" : "ltr"}
    >
      <header className="sticky top-0 z-40 border-b border-[var(--sunlit-line)] bg-[rgb(255_250_245_/_90%)] px-5 py-3 backdrop-blur-xl sm:px-7 xl:px-10">
        <div className="mx-auto flex max-w-[1380px] items-center justify-between gap-4">
          <div className="flex min-w-0 items-center gap-3">
            <Link
              aria-label={
                locale === "ar" ? "العودة إلى مساحة العمل" : "Back to workspace"
              }
            ...
            >
              ...
              <span className="hidden sm:block">
                <span className="block text-sm font-black text-[var(--sunlit-ink)]">
                  MARKOS AI
                </span>
                <span className="block text-xs font-semibold text-[var(--sunlit-muted)]">
                  {locale === "ar"
                    ? "إعدادات مساحة العمل"
                    : "Workspace settings"}
                </span>
          ...
          <div className="flex shrink-0 items-center gap-2">
            <div
              className="flex items-center rounded-xl border border-[var(--sunlit-line)] bg-white/80 p-1"
              aria-label="Language switcher"
            >
              <Link
                className={
                  locale === "ar" ? activeLanguageClass : languageClass
                }
              ...
              <Link
                className={
                  locale === "en" ? activeLanguageClass : languageClass
                }
