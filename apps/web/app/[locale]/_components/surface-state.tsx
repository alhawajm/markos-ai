...
export function SurfaceState({
  ...
  const classes =
    appearance === "luxury" ? luxuryToneClass[tone] : toneClass[tone];
  const surfaceClass =
    appearance === "luxury"
      ? `lux-card-muted ${classes.border}`
      : `shadow-card ${classes.border}`;
    ...
    <div className={`rounded-[20px] border p-4 ${surfaceClass}`}>
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex items-start gap-3">
          <div
            className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-[14px] ${classes.icon}`}
          >
            {tone === "loading" ? (
          ...
          <div>
            ...
            <p
              className={
                appearance === "luxury"
                  ? "mt-1 text-sm leading-6 text-[#C7CDD8]"
